...
# Safe deploy script for the frontend UI (rsync -> NAS) and restart nginx container
...
SSH_CHMOD=( ssh "${SSH_USER:-$NAS_USER}"@"$NAS_HOST" -p "$SSH_PORT" ${SSH_KEY:+-i "$SSH_KEY"} "chmod -R a+rX '$NAS_PATH/dist'" )
SSH_RESTART=( ssh "${SSH_USER:-$NAS_USER}"@"$NAS_HOST" -p "$SSH_PORT" ${SSH_KEY:+-i "$SSH_KEY"} "cd '$NAS_PATH' && docker compose restart web" )
...
echo "Setting permissions on remote dist"
ssh -p "$SSH_PORT" ${SSH_KEY:+-i "$SSH_KEY"} "${NAS_USER}@${NAS_HOST}" "chmod -R a+rX '${NAS_PATH}/dist'" 2>/dev/null || ssh -p "$SSH_PORT" ${SSH_KEY:+-i "$SSH_KEY"} "${NAS_USER}@${NAS_HOST}" "chmod -R a+rX '${NAS_PATH}/dist'"

# Restart web container
echo "Restarting web container on NAS"
ssh -p "$SSH_PORT" ${SSH_KEY:+-i "$SSH_KEY"} "${NAS_USER}@${NAS_HOST}" "cd '${NAS_PATH}' && docker compose restart web"

echo "Done. If you have caching or a Service Worker, hard-refresh your browser (Cmd/Ctrl+Shift+R)."