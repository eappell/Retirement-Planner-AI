...
# Deploy API on remote NAS (recommended to build on NAS)
...
# If image specified -> pull and restart using the image (assumes docker-compose.yaml uses image placeholder or we run a container directly)
if [[ -n "$IMAGE" ]]; then
