  ...
  try {
    ...
    try {
      if (providerLower === 'claude') text = await generateWithClaude(prompt);
        ...
        // Try configured model and fall back to an explicitly supported model if not found
        const configuredModel = process.env.GOOGLE_MODEL || 'gemini-2.5-flash';
        try {
          text = await generateWithGoogle(prompt);
        } catch (err) {
          console.warn('Google generation failed for configured model', configuredModel, 'error:', String(err));
          const fallbackModel = 'gemini-2.0-flash';
          if (configuredModel !== fallbackModel) {
            console.log('Attempting fallback Google model', fallbackModel);
            const orig = process.env.GOOGLE_MODEL;
            process.env.GOOGLE_MODEL = fallbackModel;
            try { text = await generateWithGoogle(prompt); } catch (err2) { console.error('Fallback Google model failed', String(err2)); throw err2; }
