...
export const runMonteCarloSimulation = (
    ...
    for (let i = 0; i < numSimulations; i++) {
        ...
        // Apply market volatility to the average returns for this specific run
        simPlan.retirementAccounts.forEach((acc: { avgReturn: number; }) => {
            const randomReturn = randomNormal(acc.avgReturn / 100, stdDev);
            acc.avgReturn = randomReturn * 100;
        });
        simPlan.investmentAccounts.forEach((acc: { avgReturn: number; }) => {
            const randomReturn = randomNormal(acc.avgReturn / 100, stdDev);
            acc.avgReturn = randomReturn * 100;
        });
